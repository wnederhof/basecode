...
internal class TemplateSolidifierTest {
    ...
    fun `Templates can be solidified using a Handlebars template and variables`() {
        ...
        assertThat(
            templateSolidifier.solidifyTemplate(template, variables),
            equalTo(expected)
        )
    }
    ...
    fun `Filenames without patterns are taken literally`() {
        assertThat(
            templateSolidifier.solidifyFilename("test.kt", listOf()),
            equalTo("test.kt")
        )
    }
    ...
    fun `Filenames with patterns are replaced`() {
        assertThat(
            templateSolidifier.solidifyFilename("[name].kt", EXAMPLE_VARIABLES),
            equalTo("blog.kt")
        )
    }
    ...
    fun `Dots in folder names are replaced by slashes`() {
        assertThat(
            templateSolidifier.solidifyFilename("[group]/[artifact]/[name]/[Name].kt", EXAMPLE_VARIABLES),
